...
import { ShieldCheck, Plus, Sparkles, Clock, AlertTriangle, FileText, CheckCircle2, FileSpreadsheet, RefreshCw } from 'lucide-react';
    ...
    <header className="h-14 bg-white border-b border-slate-200 text-slate-900 sticky top-0 z-10 shadow-2xs flex items-center justify-between px-4 sm:px-6 shrink-0">
      ...
      <div className="flex items-center gap-2">
        ...
        <button
          ...
          className="flex items-center gap-1.5 px-2.5 py-1.5 bg-emerald-50 hover:bg-emerald-100 text-emerald-800 rounded-lg text-xs font-semibold border border-emerald-300 transition-colors shadow-2xs cursor-pointer"
          title="Google Workspace (Sheets & Drive Integration & Debug Panel)"
          id="btn-google-workspace"
        >
          <FileSpreadsheet className="w-3.5 h-3.5 text-emerald-600" />
          <span className="hidden sm:inline">Sheets Settings</span>
        </button>
        ...
        <div className="flex items-center gap-2 pl-1">
          <div className="w-7 h-7 rounded-full bg-emerald-100 text-emerald-800 border border-emerald-300 flex items-center justify-center font-bold text-[11px]">
            ML
          </div>
          <div className="hidden lg:flex flex-col text-left">
            <span className="text-xs font-semibold text-slate-800 leading-tight">Madam Lim</span>
            <span className="text-[10px] text-slate-500 leading-tight">Accounts Executive, Boon Huat Hardware</span>
          </div>
        </div>
      </div>
